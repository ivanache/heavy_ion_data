...
/**
 my_code is a data-processing macro meant for processing THnSparses root files, specifically the h_Pion one in THnSparses_071617.root
...
void my_code(int NumOfCuts) {
    ...
    string headers[cutnum_option_quantity] = {"None ", "angle > 20 mrad ", "0.1 < lambda < 0.4 ", "dR > 20 mrad ", "asymmetry < 0.7 "};
    ...
    //Open the files
    TFile* fIn = new TFile("THnSparses_071617.root","READ"); //get file
    ...
    if(NumOfCuts == 0) {
        cout << "No cuts done except built-in cuts\n\n";
        //fit_y_max = 1600.0;
    }
    else if (NumOfCuts == 1) {
        SetCut(h_Pion, axis_pionAngle, 0.020, 0.5);
        std::cout << "Cuts: angle\n\n";
        //fit_y_max = 1000.0;
    }
    else if (NumOfCuts == 2) {
    ...
    func->SetParLimits(1, 0.13, 0.175); //mean
    func->SetParLimits(2, 0.006, 0.018); // width
    ...
    string table_headers[cutnum_option_quantity] = {"None ", "+angle $> 20$ mrad ", "+$0.1 < \\lambda <$ 0.4 ", "+dR $> 20$ mrad ", "+asymmetry $< 0.7$ "};
