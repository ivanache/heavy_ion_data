...
/**
 my_code is a data-processing macro meant for processing THnSparses root files, specifically the h_Pion one in THnSparses_072417.root
 the macro must be called with an int, which tells how many cuts to make cuts are made, in the following order: distance to charged particle > 0.02, asymmetry < 0.7, angle > 17 (or 18, depending on the option), Ncells > 1, distance to border > 2, distance to bad cell > 1 , lambda02 is between 0.1 and 0.4
...
// Crystal ball function, linear background model
double crystal_ball_model_linear(Double_t *x, Double_t *par) {
    double A = par[0];
    double mean = par[1];
    double sigma = par[2];
    double alpha = par[3];
    ...
    double fitval = A*ROOT::Math::crystalball_pdf(x[0], alpha, n, sigma, mean) + B*x[0] + C;
    return fitval;
}
...
void my_code(string option = "DEFAULT") {
    ...
    //Open the files
    TFile* fIn = new TFile("THnSparses_072417.root","READ"); //get file
    string rootfilename;
    ...
    int num_of_peak_params = 5;
    background = new TF1("background curve", "[0]*x + [1]", 0.08, 0.26);
    //background = new TF1("background curve", "[0]*x*x + [1]*x + [2]", 0.08, 0.26);
    //num_of_peak_params = 5;
    func = new TF1("fit", crystal_ball_model_linear,0.05,0.5,num_of_params);
    peak = new TF1("mass peak", crystal_ball_function_peak, 0.05, 0.5, num_of_peak_params);
    func->SetParNames("Integral", "Mean", "Sigma", "Alpha", "N", "Linear coeff", "Constant");
    ...
    std::cout << "\nLinear:" << func->GetParameter(5) << std::endl << "Error:" << func->GetParError(8) << std::endl;
    std::cout << "\nConstant:" << func->GetParameter(6) << std::endl << "Error:" << func->GetParError(9) << std::endl;
    func->SetLineColor(kRed);
    func->Draw("same");
