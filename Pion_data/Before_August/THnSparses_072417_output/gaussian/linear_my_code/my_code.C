...
double gaussian_model(Double_t *x, Double_t *par) {
    ...
    double fitval = A*TMath::Exp(-0.5*arg*arg)/TMath::Sqrt(2*TMath::Pi()*sigma*sigma) + B*x[0] + C;
    return fitval;
}
...
void my_code(string option = "DEFAULT") {
    ...
    func->SetParNames("Integral", "Mean", "Sigma", "Linear coeff", "Constant");
    ...
    std::cout << "\nLinear:" << func->GetParameter(3) << std::endl << "Error:" << func->GetParError(6) << std::endl;
    std::cout << "\nConstant:" << func->GetParameter(4) << std::endl << "Error:" << func->GetParError(7) << std::endl;
    func->SetLineColor(kRed);
    func->Draw("same");
