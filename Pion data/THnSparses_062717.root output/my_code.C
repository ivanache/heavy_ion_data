/**
 my_code is a data-processing macro meant for processing THnSparses root files, specifically the h_Pion one in THnSparses_062717.root
...
void my_code(int NumOfCuts) {
    ...
    //Open the files
    TFile* fIn = new TFile("THnSparses_062717.root","READ"); //get file
    ...
    if(NumOfCuts == 0) {
        ...
        SetCut(h_Pion, axis_pionDisToBorder1, 2.0, 6.0);
        std::cout << "Cuts: matched tracks, asymmetry, angle, Ncells, and DisToBorder\n\n";
    }
    else if (NumOfCuts == 6) {
        SetCut(h_Pion, axis_pionMatchedTracks1, -1.5, -0.75);
        SetCut(h_Pion, axis_pionMatchedTracks2, -1.5, -0.75);
        ...
        std::cout << "Cuts: matched tracks, asymmetry, angle, Ncells, DisToBorder, and lambda02\n\n";
    }
