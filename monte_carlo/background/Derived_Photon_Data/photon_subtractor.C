...
// Precondition: whichPhotons = "total", "leading", or "trailing"
void photon_subtractor(int NumOfSigmasFromMeanMin, int NumOfSigmasFromMeanMax, string whichPhotons = "total") {
    ...
    TH2D* histmin = 0;
    TH2D* histmax = 0;
    fInMin->GetObject(Form("%s_Evslambda", whichPhotons.c_str()), histmin);
    fInMax->GetObject(Form("%s_Evslambda", whichPhotons.c_str()), histmax);
    
    // Create a TH2D to store the data, use one of the input histograms as a template
    TH2D* histoutput = (TH2D*)histmin->Clone(Form("%s_Evslambda_subtracted", whichPhotons.c_str()));
    ...
    histoutput->SetTitle(Form("; lambda0; %s photon energy (GeV)", whichPhotons.c_str()));
    histoutput->Draw("COLZ");
    myText(.20,.97, kBlack, Form("%s Pi0 Photon Energy vs lambda0, Pt 6-20 GeV", whichPhotons.c_str()));
    myText(.35,.92, kBlack, Form("mass %i-%i sigma from mean", NumOfSigmasFromMeanMin, NumOfSigmasFromMeanMax));
    histoutput->Write(Form("%s_Evslambda", whichPhotons.c_str()));
    canvas->SaveAs(str_concat_converter(directory_name, Form("%sEvsLambda.png", whichPhotons.c_str())));
    ...
    for(int i = 0; i < num_of_intervals; i++) {
        ...
        // get the data
        fInMin->GetObject(Form("%s_Evslambda_ptmin_%2.2fGeV_ptmax_%2.2fGeV", whichPhotons.c_str(), ptmin, ptmax), histmin);
        fInMax->GetObject(Form("%s_Evslambda_ptmin_%2.2fGeV_ptmax_%2.2fGeV", whichPhotons.c_str(), ptmin, ptmax), histmax);
        ...
        histoutput->SetTitle(Form("; lambda0; %s photon energy (GeV)", whichPhotons.c_str()));
        histoutput->Draw("COLZ");
        myText(.20,.97, kBlack, Form("%s Pi0 Photon Energy vs lambda0, Pt %2.2f-%2.2f GeV", whichPhotons.c_str(), ptmin, ptmax));
        myText(.35,.92, kBlack, Form("mass %i-%i sigma from mean", NumOfSigmasFromMeanMin, NumOfSigmasFromMeanMax));
        ...
        histoutput->Write(Form("%s_Evslambda_ptmin_%2.2fGeV_ptmax_%2.2fGeV", whichPhotons.c_str(), ptmin, ptmax));
        canvas->SaveAs(str_concat_converter(directory_name, Form("%s_Evslambda_ptmin_%2.2fGeV_ptmax_%2.2fGeV.png", whichPhotons.c_str(), ptmin, ptmax)));
