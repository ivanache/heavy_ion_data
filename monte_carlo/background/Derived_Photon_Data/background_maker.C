...
void background_maker(int NumOfSigmasFromMean) {
    ...
    TFile* ratioIn = new TFile("BackgroundMC_PionSparsesOutput_angle_17mrad.root", "READ");
    ...
    // Get the signal-to-total ratios for the individual momentum intervals from the signal to toatal ratio source file (BackgroundMC_PionSparsesOutput_angle_17mrad.root)
