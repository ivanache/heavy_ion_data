/**
   This macro creates momentum-photon graphs from THnSparses_080117_MC.root
*/
...
/**
 Main function
 lambda_min is used to denote which portion of lambda0 is to be used:
...
*/
void photon_grapher(double lambda_min, double pT_min) {
    // Initialize the lambda bounds, pT lower bound, and the directory to be used based on the mins, quit the program if a min is invalid
    ...
    else if (lambda_min == 0.4){
        directory_name = "0.4<lambda/";
        lambdamin = 0.4;
        lambdamax = 2.1;
    }
    else {
        std::cout << "ERROR: input lambda_min must be 0.1 or 0.4" << std::endl;
        return;
    }
    ...
    if (pT_min == 5.0) {
        ...
        std::cout << "ERROR: input pT_option must be 5 or 10" << std::endl;
        return;
    }
