...
{
    if (argc < 4) {
      fprintf(stderr,"Please Indicate HDF5 file, mix start, mix end");
        exit(EXIT_FAILURE);
    }
    ...
    //MONEY MAKING LOOP
	  ...
	  Long64_t mix_event = imix; //artifact of original code.
    ...
    jet_pTD_hdf5->Draw();
    c->SaveAs("HDF5_jetpTD_Distribution.pdf");
    c->Clear();
    
    jet_Multiplicity_hdf5->Draw();
