...
int main(int argc, char *argv[])
{
    ...
    for (int iarg = 1; iarg < argc; iarg++) {
        ...
        for (int izt = 0; izt<nztbins; izt++){
            ...
            h_dPhi_signal[izt]->SetTitle("; #Delta#phi/#pi [rad]; entries");
            //h_dPhi_background[izt]->SetTitle("; #Delta#phi/#pi [rad]; entries");
            h_dPhi_signal[izt]->Sumw2();
            //h_dPhi_background[izt]->Sumw2();
            
            Map_signal[izt] = new TH2D(Form("Map_signal_ztmin%1.0f_ztmax%1.0f",10*ztbins[izt],10*ztbins[izt+1]), "#gamma-H [Signal] Map", n_phi_bins,-M_PI/2,3*M_PI/2, 14, -1.4, 1.4);
                ...
                for (ULong64_t itrack = 0; itrack < ntrack; itrack++) {
                    ...
                    for(int izt = 0; izt<nztbins ; izt++){
                        if(zt>ztbins[izt] and  zt<ztbins[izt+1])
                        {
                            ...
                            //if(isolation> noniso_min && isolation<noniso_max) h_dPhi_background[izt]->Fill(dphi);
                        }
                    } // end loop over bins
