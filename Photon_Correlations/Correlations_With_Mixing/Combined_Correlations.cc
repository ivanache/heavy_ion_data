...
int main(int argc, char *argv[])
{
    ...
    for (int izt = 0; izt<nztbins; izt++){
        ...
        h_dPhi_all_mixed[izt] = new TH1F(Form("dPhi_all_mixed_ztmin%1.0f_ztmax%1.0f",10*ztbins[izt], 10*ztbins[izt+1]), "", n_correlationbins,-0.5,1.5);
        h_dPhi_iso_mixed[izt]->SetTitle("; #Delta#phi/#pi [rad]; entries");
        h_dPhi_noniso_mixed[izt]->SetTitle("; #Delta#phi/#pi [rad]; entries");
        h_dPhi_all_mixed[izt]->SetTitle("; #Delta#phi/#pi [rad]; entries");
        h_dPhi_iso_mixed[izt]->Sumw2();
        h_dPhi_noniso_mixed[izt]->Sumw2();
        h_dPhi_all_mixed[izt]->Sumw2();
                            ...
                            if(zt>ztbins[izt] and  zt<ztbins[izt+1])
                            {
                                ...
                                if(isolation< iso_max){
                                    if (cluster_s_nphoton[n][1] > DNN_min && cluster_s_nphoton[n][1] < DNN_max){
                                        h_dPhi_iso_mixed[izt]->Fill(DeltaPhi/M_PI);
                                        IsoMap_mixed[izt]->Fill(DeltaPhi,DeltaEta);
                                        mixed_clusters_passed_iso[izt] += 1;
                                    }
                                }
                                if(isolation<iso_max){
                                    if (cluster_s_nphoton[n][1] > 0.0 && cluster_s_nphoton[n][1] < 0.3){ //sel deep photons
                                        h_dPhi_noniso_mixed[izt]->Fill(DeltaPhi/M_PI);
                                        AntiIsoMap_mixed[izt]->Fill(DeltaPhi,DeltaEta);
                                        mixed_clusters_passed_Antiiso[izt] += 1;
                                    }
                                }
                                Map_mixed[izt]->Fill(DeltaPhi,DeltaEta);
                                h_dPhi_all_mixed[izt]->Fill(DeltaPhi/M_PI);
                                mixed_clusters_passed[izt] += 1;
                            }//if in zt bin
                ...
                for (ULong64_t itrack = 0; itrack < ntrack; itrack++) {
                    ...
                    for(int izt = 0; izt<nztbins ; izt++){
                        if(zt>ztbins[izt] and  zt<ztbins[izt+1])
                        {
                            ...
                            if(isolation< iso_max){
                                if (cluster_s_nphoton[n][1] > DNN_min && cluster_s_nphoton[n][1] < DNN_max){
                                    h_dPhi_iso_same[izt]->Fill(DeltaPhi/M_PI);
                                    IsoMap_same[izt]->Fill(DeltaPhi,DeltaEta);
                                    same_clusters_passed_iso[izt] += 1;
                                }
                            }
                            if(isolation> noniso_min && isolation<noniso_max){
                                if (cluster_s_nphoton[n][1] > DNN_min && cluster_s_nphoton[n][1] < DNN_max){
                                    h_dPhi_noniso_same[izt]->Fill(DeltaPhi/M_PI);
                                    AntiIsoMap_same[izt]->Fill(DeltaPhi,DeltaEta);
                                    same_clusters_passed_Antiiso[izt] += 1;
                                }
                            }
                            Map_same[izt]->Fill(DeltaPhi,DeltaEta);
                            same_clusters_passed[izt] += 1;
                        }
                    } // end loop over bins
    ...
    // Solve for the correlations, but first normalize the same and mixed event correlations
    for (int izt = 0; izt<nztbins; izt++){
        // Normalize
        // Same event normalization
        divide_histograms1D_byscalar(h_dPhi_iso_same[izt], same_clusters_passed_iso[izt]);
        divide_histograms1D_byscalar(h_dPhi_noniso_same[izt], same_clusters_passed_Antiiso[izt]);
        
        divide_histograms2D_byscalar(Map_same[izt], same_clusters_passed[izt]);
        divide_histograms2D_byscalar(IsoMap_same[izt], same_clusters_passed_iso[izt]);
        divide_histograms2D_byscalar(AntiIsoMap_same[izt], same_clusters_passed_Antiiso[izt]);
        
        //Mixed Event normalization
        divide_histograms1D_byscalar(h_dPhi_iso_mixed[izt], h_dPhi_iso_mixed[izt]->GetBinContent(h_dPhi_iso_mixed[izt]->GetXaxis()->FindBin(0.0)));
        divide_histograms1D_byscalar(h_dPhi_noniso_mixed[izt], h_dPhi_noniso_mixed[izt]->GetBinContent(h_dPhi_noniso_mixed[izt]->GetXaxis()->FindBin(0.0) ));
        
        divide_histograms2D_byscalar(Map_mixed[izt], Map_mixed[izt]->GetBinContent(Map_mixed[izt]->GetXaxis()->FindBin(0.0), Map_mixed[izt]->GetYaxis()->FindBin(0.0)));
        divide_histograms2D_byscalar(IsoMap_mixed[izt], IsoMap_mixed[izt]->GetBinContent(IsoMap_mixed[izt]->GetXaxis()->FindBin(0.0), IsoMap_mixed[izt]->GetYaxis()->FindBin(0.0)));
        divide_histograms2D_byscalar(AntiIsoMap_mixed[izt], AntiIsoMap_mixed[izt]->GetBinContent(AntiIsoMap_mixed[izt]->GetXaxis()->FindBin(0.0), AntiIsoMap_mixed[izt]->GetYaxis()->FindBin(0.0)));
    ...
    for (int izt = 0; izt<nztbins; izt++){
        Map_corr[izt]->Write();
        Map_corr[izt]->Draw("SURF2");
        canvas.SaveAs(Form("Map_corr_ztmin%1.0f_ztmax%1.0f.png",10*ztbins[izt],10*ztbins[izt+1]));
        canvas.Clear();
        IsoMap_corr[izt]->Write();
        IsoMap_corr[izt]->Draw("SURF2");
        canvas.SaveAs(Form("IsoMap_corr_ztmin%1.0f_ztmax%1.0f.png",10*ztbins[izt],10*ztbins[izt+1]));
        canvas.Clear();
        AntiIsoMap_corr[izt]->Write();
        AntiIsoMap_corr[izt]->Draw("SURF2");
        canvas.SaveAs(Form("AntiIsoMap_corr_ztmin%1.0f_ztmax%1.0f.png",10*ztbins[izt],10*ztbins[izt+1]));
        canvas.Clear();
    }
    for (int izt = 0; izt<nztbins; izt++){
        h_dPhi_corr_iso_over_all[izt]->SetMinimum(0.0);
        h_dPhi_corr_iso_over_all[izt]->Write();
        h_dPhi_corr_iso_over_all[izt]->Draw();
        canvas.SaveAs(Form("dPhi_corr_iso_over_all_ztmin%1.0f_ztmax%1.0f.png",10*ztbins[izt],10*ztbins[izt+1]));
        canvas.Clear();
    }
    for (int izt = 0; izt<nztbins; izt++){
        Map_iso_over_all_corr[izt]->Write();
        Map_iso_over_all_corr[izt]->Draw("SURF2");
        canvas.SaveAs(Form("Map_iso_over_all_corr_ztmin%1.0f_ztmax%1.0f.png",10*ztbins[izt],10*ztbins[izt+1]));
        canvas.Clear();
    }
    fout->Close();
